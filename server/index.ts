...
io.on("connection",socket=>{
  ...
  socket.on("session:reset",callback=>{const session=currentSession(socket.id),token=socketTokens.get(socket.id);if(session?.mode==="online"&&token)leaveOnlineRoom(session,token);else{if(session){clearTurnTimer(session);if(session.attributeTimer)clearTimeout(session.attributeTimer);if(session.reconnectTimer)clearTimeout(session.reconnectTimer);if(session.rematchTimer)clearTimeout(session.rematchTimer);if(session.pendingReaction?.timer)clearTimeout(session.pendingReaction.timer)}if(token)sessions.delete(token)}socketTokens.delete(socket.id);const resetState:SessionState={phase:"title"};callback({ok:true,state:resetState});sendState(socket.id,resetState)});
