...
function publicStateForToken(session:StoredSession,token:string):SessionState{
  ...
  if(side==="cpu"){const playerAttribute=result.playerAttribute;result.playerAttribute=result.cpuAttribute;result.cpuAttribute=playerAttribute;if(result.battle){const battle=result.battle,hostPlayer=battle.player,guestPlayer=battle.cpu;const guestHand=structuredClone(session.cpuHand);result.battle={...battle,activePlayer:battle.activePlayer==="player"?"cpu":"player",winner:battle.winner?battle.winner==="player"?"cpu":"player":undefined,player:{...guestPlayer,hand:guestHand,discard:structuredClone(session.cpuDiscard)},cpu:{...hostPlayer,handCount:hostPlayer.hand.length}};decorateHand(result,"player",result.battle.player.hand)}}
  return result;
}
...
function cleanupUnits(state:SessionState,side:Side):void{const owner=stateForSide(state,side),dead=owner.shikigami.filter(unit=>unit.hp<=0);for(const unit of dead){state.battle!.log.push(`${unit.name} retired.`);owner.retiredShikigami.push({...structuredClone(unit),curses:[],nextDamageReduction:0,nextAttackBonus:0});if(state.battle!.terrain?.id==="terrain_yomi_road"){owner.mp=Math.min(30,owner.mp+1);state.battle!.log.push(`Yomi Road granted ${side==="player"?"Player":"CPU"} 1 MP.`)}}owner.shikigami=owner.shikigami.filter(unit=>unit.hp>0)}
...
function resolveUtilityCard(session:StoredSession,side:Side,definition:Exclude<CardEffectDefinition,AttackDefinition|DefenseDefinition>,target:CardTarget|undefined,choice?:string):void{
  ...
  else if(definition.type==="generate"){if(attributeForSide(state,side)===definition.requiredAttribute){actor.mp=Math.min(30,actor.mp+definition.successMp);rotateAttribute(state,side,definition.steps)}else actor.mp=Math.min(30,actor.mp+definition.failureMp)}
  ...
  else if(definition.type==="restore"){actor.hp=Math.min(30,actor.hp+definition.hp);actor.mp=Math.min(30,actor.mp+definition.mp)}
  ...
  else if(definition.type==="sacrifice"){const unit=manual?findAllyUnitTarget(state,side,target):actor.shikigami[0];if(unit){actor.shikigami=actor.shikigami.filter(item=>item.instanceId!==unit.instanceId);actor.hp=Math.min(30,actor.hp+definition.heal);actor.mp=Math.min(30,actor.mp+definition.mp)}}
...
function executeCard(session:StoredSession,side:Side,index:number,externalTarget?:CardTarget,choice?:string):{ok:boolean;message?:string;paused?:boolean}{
  ...
  if(definition.type==="attack"){
    ...
    if(state.mode==="online"&&defenseCards(session,otherSide(side)).length){const predictions=targets.map(target=>{const targetElement=targetAttribute(state,side,target);return definition.baseDamage+(match?1:0)+(targetElement&&overcomes[cardElement!]===targetElement?2:0)});beginReaction(session,otherSide(side),card.name,side==="player"?(session.hostName??"Host"):(session.guestName??"Guest"),targets,predictions,(defense,defenseTarget)=>{resolve(defense,defenseTarget);if(!finishIfNeeded(state))battle.phase="card_use"});return {ok:true,paused:true}}
    if(side==="cpu"&&defenseCards(session,"player").length){const predictions=targets.map(target=>{const targetElement=targetAttribute(state,side,target);return definition.baseDamage+(match?1:0)+(targetElement&&overcomes[cardElement!]===targetElement?2:0)});beginReaction(session,"player",card.name,"CPU",targets,predictions,(defense,defenseTarget)=>{resolve(defense,defenseTarget);if(!finishIfNeeded(state))continueCpuTurn(session)});return {ok:true,paused:true}}
...
function resolveShikigamiAttack(state:SessionState,side:Side,unit:ShikigamiState,target:UnitTarget,hits:number[],defense?:DefenseDefinition,selectedTarget?:DefenseTarget):void{
  ...
  if(shikigamiIsAlive(state,side,unit)){
    if(abilityEnabled&&unit.shikigamiId==="shikigami_kanko"){actor.mp=Math.min(30,actor.mp+1);battle.log.push("霊気集めによりMPが1増加した。")}
...
function runOneShikigamiAction(session:StoredSession,side:Side,unit:ShikigamiState,resume?:()=>void):{paused:boolean}{
  ...
  if(abilityEnabled&&unit.shikigamiId==="shikigami_genki"&&unit.hp<=unit.maxHp/2&&unit.nextDamageReduction<2){unit.nextDamageReduction=2;battle.log.push(`${unit.name}は甲羅籠りを行った。`);return {paused:false}}
...
function processTurnStart(state:SessionState,side:Side):void{
  ...
  if(actor.barrier?.id==="barrier_spirit_vein"){if(actor.barrier.skipNextTrigger)actor.barrier.skipNextTrigger=false;else{actor.mp=Math.min(30,actor.mp+1);battle.log.push(`霊脈結界により${side==="player"?"プレイヤー":"CPU"}のMPが1増加した。`)}}
...
function cpuCardScore(session:StoredSession,card:CardView,definition:CardEffectDefinition):number{
  ...
  else if(definition.type==="restore")score+=Math.min(30-actor.hp,definition.hp)*aiValue("ai_heal_player")+Math.min(30-actor.mp,definition.mp)*aiValue("ai_gain_mp");
...
function continueCpuTurn(session:StoredSession):void{
  ...
  battle.turnNumber+=1;battle.activePlayer="player";battle.phase="card_use";battle.player.cost=5;processTurnStart(state,"player");drawToLimit(battle.player.hand,5);battle.log.push(`第${battle.turnNumber}ターン開始。手札とコストを更新した。`);refreshPlayability(state);armPlayerTurnTimer(session);
}
...
function initializeBattle(session:StoredSession):void{
  const hand=drawCards(5);session.cpuHand=drawCards(5);session.state.phase="battle";session.state.battle={turnNumber:1,activePlayer:"player",phase:"card_use",player:{hp:30,mp:0,cost:5,curses:[],nextDamageReduction:0,shikigami:[],retiredShikigami:[],hand,discard:[]},cpu:{hp:30,mp:0,cost:5,curses:[],nextDamageReduction:0,shikigami:[],retiredShikigami:[],handCount:session.cpuHand.length},log:["Match started. Both players drew five cards."]};refreshPlayability(session.state);if(session.mode!=="online")armPlayerTurnTimer(session);
}
...
function completeOnlineTurn(session:StoredSession,side:Side):void{
  ...
  const next=otherSide(side);if(side==="cpu")battle.turnNumber+=1;battle.activePlayer=next;battle.phase="card_use";const actor=stateForSide(state,next);actor.cost=5;processTurnStart(state,next);drawToLimit(handForSide(session,next),5);battle.cpu.handCount=session.cpuHand.length;refreshPlayability(state);armOnlineTurnTimer(session);
}
...
io.on("connection",socket=>{
  ...
  socket.on("session:reset",callback=>{const session=currentSession(socket.id),token=socketTokens.get(socket.id);if(session?.mode==="online"&&token)leaveOnlineRoom(session,token);else{if(session?.pendingReaction?.timer)clearTimeout(session.pendingReaction.timer);if(token)sessions.delete(token)}socketTokens.delete(socket.id);const resetState:SessionState={phase:"title"};callback({ok:true,state:resetState});sendState(socket.id,resetState)});
